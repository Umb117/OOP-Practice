namespace Itmo.ObjectOrientedProgramming.Lab2.Subjects.Builders;

public abstract class SubjectBuilderBase : ISubjectBuilder
{
    private static int _generalId = 0;
    private readonly int _id;

    public int? OriginalId { get; init; }

    private readonly int _authorId;
    private string _name;
    private readonly List<ILaboratory> _labs;
    private readonly List<ILection> _lections;
    private int _examScores;

    public ISubjectBuilder WithTitle(IRenderable title)
    {
        _title = title;
        return this;
    }

    public ISubjectBuilder AddSection(IRenderable section)
    {
        _sections.Add(section);
        return this;
    }

    public ISubjectBuilder WithFooter(IRenderable footer)
    {
        _footer = footer;
        return this;
    }

    public ISubject Build()
    {
        return Build(
            _title ?? throw new ArgumentNullException(),
            _sections.ToArray(),
            _footer);
    }

    protected abstract ISubject Build(
        IRenderable title,
        IReadOnlyCollection<IRenderable> sections,
        IRenderable? footer);
}